...
/**
 * Creates a minimal DOCX buffer with caller-provided WordprocessingML body XML.
 */
export async function createTestDocxFromBodyXml(bodyXml: string): Promise<Buffer> {
  const zip = new JSZip();
...
export async function readDocxXml(buffer: Buffer, path: string): Promise<string> {
  const zip = await JSZip.loadAsync(buffer);
  const file = zip.file(path);
  if (!file) {
    throw new Error(`${path} was not found in DOCX`);
  }
