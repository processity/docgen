...
const SECRET_NAMES = {
  ...
  RECONNECT_SF_CLIENT_SECRET: 'DOCGEN-RECONNECT-SF-CLIENT-SECRET',
} as const;
...
export async function loadSecretsFromKeyVault(
  ...
  try {
    ...
    const secretPromises = Object.entries(SECRET_NAMES).map(async ([key, secretName]) => {
      ...
      if ((key === 'RECONNECT_AAD_CLIENT_SECRET' || key === 'RECONNECT_SF_CLIENT_SECRET') && !options.includeReconnectSecret) {
        return { key, value: undefined };
      }
      try {
      ...
      if (value && value.trim() !== '') {
        switch (key) {
          ...
          case 'SFDX_AUTH_URL':
            ...
            break;
          case 'SF_ACCESS_TOKEN':
            secrets.sfAccessToken = value;
            break;
          case 'SF_INSTANCE_URL':
            secrets.sfInstanceUrl = value;
            break;
          ...
          case 'RECONNECT_AAD_CLIENT_SECRET':
            ...
            break;
          case 'RECONNECT_SF_CLIENT_SECRET':
            secrets.reconnectSfClientSecret = value;
            break;
        }
      }
    }
